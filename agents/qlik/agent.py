...
from .tasks import QlikListReloadsTask
...
async def main() -> None:
    ...
    registry.register("qlik", QlikConfiguration)
    agent_config = registry.lookup("qlik", QlikConfiguration)
    ...
    async with open_nursery() as n:
        n.start_soon(
            PeriodicLoop(
                ...
                task=QlikListReloadsTask(nursery=n, outbound_channel=event_queue_send),
            ).run,
        )

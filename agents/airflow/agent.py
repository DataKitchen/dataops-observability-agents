...
from .job_runs import AirflowListRunsTask
...
async def main() -> None:
    ...
    agent_config = registry.lookup("airflow", AirflowConfiguration)
    core_config = registry.lookup("core", CoreConfiguration)
    auth_config = load_agent_credentials()
    if not isinstance(auth_config, UsernamePasswordConfiguration):
        LOG.error(
            "Airflow agent only supports basic authentication. See docs for more help.",
        )
        sys.exit()
    ...
    async with open_nursery() as n:
        n.start_soon(
            PeriodicLoop(
                ...
                task=AirflowListRunsTask(nursery=n, outbound_channel=event_queue_send),
            ).run,
        )
